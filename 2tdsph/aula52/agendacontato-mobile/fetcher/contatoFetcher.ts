...
const contatoFetcherSalvar =
 (contato : Contato, token? : string ) : Promise<void> => {
    const config : AxiosRequestConfig = {headers: {"Authorization": `Bearer ${token}`}};
    console.log("Headers: ", config);
    return apiBase.post( "/contato", contato, config );
}
...
const contatoFetcherLer = async (token? : string ) : Promise<Contato[]> => { 
    const config : AxiosRequestConfig = {headers: {"Authorization": `Bearer ${token}`}};
    console.log("Headers: ", config);
    const response = await apiBase.get("/contato", config);
    const listaContatos = [];
        for ( const objContato of response.data ){  
...
export {contatoFetcherSalvar, contatoFetcherLer, 
    ...
    contatoFetcherImageUpload, SalvarCallback, 
    ApagarCallback, AtualizarCallback, ImageUploadCallback}; 