...
import { loginFetcherLogar, loginFetcherRegistrar, LogarCallback } from "../fetcher/loginFetcher";
...
const loginServicoRegistrar = 
    ( usuario : Usuario, callback : LogarCallback ) : void => {
        usuarioSchema.validate( usuario, {abortEarly: false} )
        .then(()=>{
            loginFetcherRegistrar( usuario, callback );
        })
        .catch((errors : ValidationError)=>{
            const usuarioErros : UsuarioErro = {}
            errors.inner.forEach( ( err : ValidationError ) => {
...
export {loginServicoLogar, loginServicoRegistrar};