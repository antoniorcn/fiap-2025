...
const useContatoControl = () => { 
    ...
    const pegarFoto = () => {
        ...
        .then((resultado : ImagePicker.ImagePickerResult) => { 
            if (!resultado.canceled) { 
                if (resultado.assets.length > 0) { 
                    ...
                    contatoImageUpload( resultado.assets[0], imgInfo, imagemCallback );
                }
            }
        })
        .catch( ( erro : any )=>{
    ...
    return { loading, mensagem, sucesso,
        ...
        salvar, ler, apagar, atualizar, setContato, handleContato, pegarFoto};
}
