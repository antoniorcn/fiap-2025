...
import { apagarApi, atualizarApi, CallBackApagar, CallBackAtualizar, 
    CallBackCarregar, CallBackSalvar, carregarApi, salvarApi, enviarImagemApi } from '../fetcher/contatoFetcher';
...
const carregarContatos = (carregarCallBack : CallBackCarregar, token? : string ) => { 
    carregarApi( carregarCallBack, token );
}

const enviarImagemContato = (contatoId : number, imagem : ImagePickerAsset ) : Promise<boolean> => { 
    return  enviarImagemApi( contatoId, imagem );
}
