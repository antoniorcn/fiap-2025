...
const useContatoControl = ( navigation : any ) => { 
    ...
    const contatoQuery = useQuery({
        queryKey: ["contatos"],
        queryFn: () => carregarContatos( token??"" )
    })
    ...
    return {
        contato, contatoLista : contatoQuery.data, 
        handlerInput, salvar, carregar, apagar, atualizar,
        carregarImageMediaLibrary, carregarImageCamera,
        loading : contatoQuery.isLoading || contatoQuery.isFetching, 
        mensagem, contatoErro, status, photo
    } 
}
