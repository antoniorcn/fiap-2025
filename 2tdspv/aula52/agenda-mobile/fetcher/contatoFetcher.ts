...
const contatoApiLoad = async ( token? : string) : Promise<Contato[]> => {    
    console.log("contatoApiLoad(): acionado");
    const response = await apiBase.get("/contato", {headers: {"Authorization": `Bearer ${token}`}})
    const lista : Contato[] = [...response.data];
    return lista;
}
